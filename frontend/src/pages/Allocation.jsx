...
function Allocation() {
  ...
  // Distinguish role if needed; here we assume Admin view
  // If we needed to show only student's own, we could have a role state
  // But per requirements, Student Allocation only shows logged-in student's allocation
  ...
  const fetchAllocations = async () => {
    ...
    try {
      if (isAdmin) {
        // Fetch allocations, students, and rooms concurrently
        ...
        // Backend returns an array even for 'my/' endpoint based on requirements
        setAllocations(data);
      }
    } catch (err) {
      console.error(err);
      setError("Failed to fetch allocations. Ensure backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="alloc-container">
      <div className="alloc-header">
        <h2>{isAdmin ? "Room Allocations Management" : "My Room Allocation"}</h2>
        {isAdmin && (
                ...
                {allocations.length > 0 ? (
                  allocations.map((alloc) => (
                    <tr key={alloc.id || Math.random()}>
                      {isAdmin && <td>{alloc.id}</td>}
                      <td className="alloc-fw-bold">{alloc.student}</td>
                      <td>
                        <span className="alloc-room-badge">
                          {alloc.room}
                        </span>
                      </td>
                  ...
                  <tr>
                    <td colSpan={isAdmin ? 5 : 3} className="alloc-empty">
                ...
                >
                  ...
                  {roomsList.map(r => (
                    <option key={r.id} value={r.room_number}>
                      {r.room_number} - {r.capitalize ? r.capitalize(r.room_type) : r.room_type} {r.available ? '(Available)' : '(Occupied)'}
                    </option>
                  ))}
                </select>
              </div>
